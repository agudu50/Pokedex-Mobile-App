...
export default function PokemonFilterHeader({
  ...
  return (
    <View style={styles.headerContainer}>
      {/* Top Header Bar with Menu / Back and Title */}
      <View style={styles.topBar}>
        ...
        <Text style={styles.titleText}>Kanto</Text>
        <View style={{ width: 26 }} />
      </View>
      ...
      {/* Filter and Sort Pills (as shown in the screenshots) */}
      <View style={styles.filterRow}>
        {/* Type Filter Button */}
        <TouchableOpacity
          style={[styles.filterButton, selectedType !== 'all' && styles.filterButtonActive]}
          onPress={() => {
            // Cycle through or open modal - for quick switching, let's open quick selector
        ...
        >
          <Text style={styles.filterButtonText}>
            {selectedType === 'all' ? 'Todos os tipos' : TYPE_CONFIG[selectedType]?.name || selectedType}
          </Text>
        ...
        >
          <Text style={styles.filterButtonText}>
            {sortBy === 'asc' ? 'Menor número' : sortBy === 'desc' ? 'Maior número' : 'Nome (A-Z)'}
          </Text>
          <Ionicons name="chevron-down" size={16} color="#FFFFFF" style={styles.filterChevron} />
        </TouchableOpacity>
      </View>

      {/* Horizontal Type Scroll for quick filtering */}
      <ScrollView
        horizontal
